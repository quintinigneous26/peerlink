...
from typing import Optional, Dict, Deque, Callable, Awaitable, List
...
class MplexFrame:
    ...
    def pack(self) -> bytes:
        ...
        # Only MESSAGE frames have data payload
        if self.flag == MplexFlag.MESSAGE and self.data:
        ...
        if flag == MplexFlag.MESSAGE:
            ...
            if remaining:
                try:
                    ...
                    if len_len + length <= len(remaining):
                        frame_data = remaining[len_len:len_len + length]
                        data_offset += len_len + length
                except (MplexProtocolError, ValueError):
                    # Incomplete data, return empty for now
                    pass

        return cls(flag=flag, stream_id=stream_id, data=frame_data), data_offset
...
class MplexSessionPool:
    """
    Pool of Mplex sessions for connection reuse.
    ...
    def __init__(
        ...
        self._sessions: Dict[str, MplexSession] = {}
        ...
        async with self._lock:
            ...
            if key in self._sessions:
                ...
                if not session.is_closed:
                    ...
                    del self._session_last_used[key]

            # Create new session
            session = await MplexSession.create_client(host, port)
            self._sessions[key] = session
            self._session_last_used[key] = time.time()

            return session

    async def return_session(self, session: 'MplexSession') -> None:
        """Return a session to the pool."""
        # Sessions stay in the pool for reuse
        # Cleanup task will remove idle sessions
        pass
            ...
            try:
                ...
                async with self._lock:
                    ...
                    for key in to_remove:
                        ...
                        logger.debug(f"Cleaned up idle session: {key}")

            except asyncio.CancelledError:
                break
            except Exception as e:
                logger.error(f"Session pool cleanup error: {e}")
...
class MplexSession:
    ...
    def __init__(
        ...
        # Priority queues for fair scheduling
        self._write_queues: Dict[StreamPriority, List[int]] = {
    ...
    def statistics(self) -> dict:
        ...
        return {
            ...
            "streams_closed": self._streams_closed,
    ...
    async def open_stream(
        ...
        async with self._streams_lock:
            ...
            self._write_queues[priority].append(stream_id)
        ...
        logger.debug(f"Opened new stream: {stream_id} (priority={priority.name})")
                    ...
                    if flag == MplexFlag.MESSAGE:
                        ...
                        if remaining:
                            try:
                                ...
                                # Not enough data yet for MESSAGE frame
                                continue
